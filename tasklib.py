...
def gaincal (cfg):
    ...
    for k in 'append preavg minblperant minsnr solnorm degamp degphase'.split ():
        if cfg.gaintype.upper() != 'BPOLY' and 'deg' in k:
            continue
        else:
    ...
    if solkws['type'] == 'GSPLINE':
        ...
        for skip in ['phaseonly', 'minblperant', 'preavg', 'minsnr', 'apmode', 'type']:  # remove extra keywords
            blah = solkws.pop(skip)
